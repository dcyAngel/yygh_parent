...
import cn.dcy.yygh.client.HospitalFeignClient;
import cn.dcy.yygh.common.exception.YyghException;
import cn.dcy.yygh.common.helper.HttpRequestHelper;
...
public class OrderServiceImpl extends ServiceImpl<OrderMapper, OrderInfo> implements OrderService {
    ...
    public boolean cancelOrder(Long orderId) {
        ...
        OrderInfo orderInfo = baseMapper.selectById(orderId);
        //判断是否取消
        DateTime quiitTime = new DateTime(orderInfo.getQuitTime());
        if(quiitTime.isBeforeNow()){
            throw new YyghException(ResultCodeEnum.TIME_NO);
        }
        //调用医院接口实现取消预约
        //调用医院接口实现预约取消
